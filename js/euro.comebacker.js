...
$(document).ready(function () {
    ...
    $(document).mouseleave(function () {
        if (isComebackerVisible() && !timerId) {
            startTimer();
        }
    });

    function isComebackerVisible() {
        return $('.e-comebacker').css('display') === 'block';
    }
    ...
    function startTimer() {
        ...
        function updateTimer() {
            var min = paddZero(Math.floor(time / 60));
            var sec = paddZero(time % 60);

            $('.modal-offer__timer-time .__min').text(min);
            $('.modal-offer__timer-time .__sec').text(sec);
        }
